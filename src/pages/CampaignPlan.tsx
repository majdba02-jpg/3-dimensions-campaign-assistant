          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-3">
            ...
            <div className="bg-white p-2.5 rounded-xl border border-slate-200/80 text-center">
              ...
              <div className="flex items-baseline justify-center gap-1">
                ...
                <span className="text-[10px] text-slate-400 font-semibold">rec.</span>
              </div>
              <span className="text-[9px] font-bold block mt-0.5 text-slate-500">
                {plan.calendar.filter((c) => c.format === 'Story').length > 0
                  ? `(${plan.calendar.filter((c) => c.format === 'Story').length} Scheduled)`
                  : `(Not Scheduled)`}
              </span>
            </div>
          </div>
