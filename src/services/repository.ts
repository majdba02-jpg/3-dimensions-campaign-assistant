...
export interface StorageRepository {
  ...
  // Plans
  getCampaignPlan(campaignId: string, planVersionId?: string): Promise<CampaignPlan | null>;
  saveCampaignPlan(plan: CampaignPlan): Promise<void>;

  // Content Assets
  getContentAssets(campaignId?: string, planVersionId?: string): Promise<ContentAsset[]>;
...
class IndexedDBRepository implements StorageRepository {
  ...
  async duplicateCampaign(campaignId: string): Promise<{
    ...
    const newPlanVersionId = origPlan ? `pv_${newCampaignId}_${Date.now()}` : undefined;
    const newBrief: CampaignBrief = {
      ...
      createdAt: nowIso,
      updatedAt: nowIso,
      workflowStage: origBrief.workflowStage || (origPlan ? 'plan_generated' : 'brief'),
      activePlanVersionId: newPlanVersionId,
  ...
  async getCampaignPlan(campaignId: string, planVersionId?: string): Promise<CampaignPlan | null> {
    if (planVersionId) {
      const byVersion = await getByIdFromStore<CampaignPlan>(STORES.CAMPAIGN_PLANS, planVersionId);
      if (byVersion) return byVersion;
    }
    const direct = await getByIdFromStore<CampaignPlan>(STORES.CAMPAIGN_PLANS, campaignId);
    if (direct) {
      if (!planVersionId || direct.planVersionId === planVersionId) return direct;
    }
    const allPlans = await getAllFromStore<CampaignPlan>(STORES.CAMPAIGN_PLANS);
    const matching = allPlans.filter((p) => p.campaignId === campaignId || p.id === campaignId);
    if (matching.length === 0) return null;
    if (planVersionId) {
      const vMatch = matching.find((p) => p.planVersionId === planVersionId || p.id === planVersionId);
      if (vMatch) return vMatch;
    }
    return matching.sort((a, b) => new Date(b.updatedAt || b.createdAt || 0).getTime() - new Date(a.updatedAt || a.createdAt || 0).getTime())[0];
  }

  async saveCampaignPlan(plan: CampaignPlan): Promise<void> {
    const planVersionId = plan.planVersionId || plan.id || `pv_${plan.campaignId}_${Date.now()}`;
    const normalizedPlan: CampaignPlan = {
      ...plan,
      id: planVersionId,
      planVersionId: planVersionId,
    };
    await saveToStore(STORES.CAMPAIGN_PLANS, normalizedPlan);

    // Keep brief activePlanVersionId & workflowStage in sync
    const brief = await getByIdFromStore<CampaignBrief>(STORES.CAMPAIGNS, plan.campaignId);
    if (brief) {
      const shouldUpdate = brief.activePlanVersionId !== planVersionId || brief.workflowStage !== 'plan_generated';
      if (shouldUpdate) {
        await saveToStore(STORES.CAMPAIGNS, {
          ...brief,
          activePlanVersionId: planVersionId,
          workflowStage: 'plan_generated',
          status: brief.status === 'Draft' ? 'In Progress' : brief.status,
  ...
  async getContentAssets(campaignId?: string, planVersionId?: string): Promise<ContentAsset[]> {
    const assets = await getAllFromStore<ContentAsset>(STORES.CONTENT_ASSETS);
    if (!campaignId) return assets;
    const campaignAssets = assets.filter((a) => a.campaignId === campaignId);
    if (planVersionId) {
      const versionAssets = campaignAssets.filter((a) => a.planVersionId === planVersionId);
      if (versionAssets.length > 0) return versionAssets;
    }
  ...
  async getFeedbackMemory(): Promise<FeedbackMemoryItem[]> {
    const rawItems = await getAllFromStore<any>(STORES.FEEDBACK_MEMORY);
    return rawItems.map((item) => {
      // Determine feedbackType
      let feedbackType: FeedbackType = item.feedbackType;
      if (!feedbackType) {
        if (item.rating === 'Positive') feedbackType = 'Positive Preference';
        else if (item.rating === 'Negative') feedbackType = 'Avoid / Negative';
        else feedbackType = 'Brand / Style Rule';
      }
      ...
      // Determine source
      const source = item.source || 'Content Review';

      // Determine scope
      const format = item.format || item.contentFormat;
      const languages =
        item.languages ||
  ...
  async saveFeedbackMemory(item: FeedbackMemoryItem): Promise<void> {
    const normalized: FeedbackMemoryItem = {
      ...item,
      isActive: item.isActive !== undefined ? item.isActive : true,
      updatedAt: new Date().toISOString(),
    };
    return saveToStore(STORES.FEEDBACK_MEMORY, normalized);
  }
  ...
  async initializeSeedData(): Promise<void> {
    ...
    // 6. Idempotent Migration: Ensure workflowStage, activePlanVersionId, and asset scoping for all campaigns
    const allBriefs = await getAllFromStore<CampaignBrief>(STORES.CAMPAIGNS);
    const allPlans = await getAllFromStore<CampaignPlan>(STORES.CAMPAIGN_PLANS);
    const allAssets = await getAllFromStore<ContentAsset>(STORES.CONTENT_ASSETS);

    for (const brief of allBriefs) {
      let needsBriefSave = false;
      const updatedBrief = { ...brief };

      const matchingPlan = allPlans.find(
        (p) =>
          p.campaignId === brief.id ||
          p.id === brief.id ||
          (brief.activePlanVersionId && (p.id === brief.activePlanVersionId || p.planVersionId === brief.activePlanVersionId))
      );

      if (matchingPlan) {
        const planVersionId = matchingPlan.planVersionId || matchingPlan.id || `pv_${brief.id}`;
        if (matchingPlan.id !== planVersionId || matchingPlan.planVersionId !== planVersionId) {
          const normalizedPlan: CampaignPlan = {
            ...matchingPlan,
            id: planVersionId,
            planVersionId: planVersionId,
        ...
        if (updatedBrief.activePlanVersionId !== planVersionId) {
          updatedBrief.activePlanVersionId = planVersionId;
          needsBriefSave = true;
        }
        if (updatedBrief.workflowStage !== 'plan_generated') {
          updatedBrief.workflowStage = 'plan_generated';
          needsBriefSave = true;
        }

        // Migrate matching assets to have this planVersionId
        const briefAssets = allAssets.filter((a) => a.campaignId === brief.id);
        for (const ast of briefAssets) {
          if (!ast.planVersionId || ast.planVersionId !== planVersionId) {
            await saveToStore(STORES.CONTENT_ASSETS, {
              ...ast,
              planVersionId: planVersionId,
            });
          }
        ...
        // No generated plan yet - set workflow stage appropriately
        if (updatedBrief.strategicDirections && updatedBrief.strategicDirections.length > 0) {
          const targetStage = updatedBrief.selectedDirectionId ? 'direction_selected' : 'directions_generated';
          if (updatedBrief.workflowStage !== targetStage) {
            updatedBrief.workflowStage = targetStage;
            needsBriefSave = true;
          }
        } else if (updatedBrief.assumptions && updatedBrief.assumptions.length > 0) {
          if (updatedBrief.workflowStage !== 'assumptions_resolved') {
            updatedBrief.workflowStage = 'assumptions_resolved';
            needsBriefSave = true;
          }
        } else if (!updatedBrief.workflowStage) {
          updatedBrief.workflowStage = 'brief';
          needsBriefSave = true;
        }
      }

      if (needsBriefSave) {
        await saveToStore(STORES.CAMPAIGNS, updatedBrief);
      }
    }
  }
}
